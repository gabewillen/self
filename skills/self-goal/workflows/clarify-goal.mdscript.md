...
* if live proof is required and `{{primary_user_action}}` is empty
  * run [Prepare Prompt Return Script](../../self-common/workflows/return-script.mdscript.md#prepare-prompt-return-script) with the resume heading `Clarify Goal` and `{{primary_user_action}}` as the requested value
