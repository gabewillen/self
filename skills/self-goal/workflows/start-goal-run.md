...
* if `{{loop_driver}}` is empty
  * set `{{loop_driver}}` to `self-hooks` when `{{skip_goal_hooks}}` is false, otherwise `harness-goal`
...
* when `{{skip_goal_hooks}}` is `true`, document in Goal Contract that harness `/goal` drives multi-round continuation and self-goal hooks are skipped
