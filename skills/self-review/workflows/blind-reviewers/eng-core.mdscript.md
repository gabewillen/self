...
* set `{{rules_file}}` to the absolute path of `../../references/engineering-rules/core.rules.md` relative to this skill file
* if `{{review_skill_root}}` is set
  * set `{{rules_file}}` to `{{review_skill_root}}/references/engineering-rules/core.rules.md`
* if `{{rules_file}}` is still relative or missing
  * resolve it from this file's directory as `../../references/engineering-rules/core.rules.md`
