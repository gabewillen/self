...
- use self-review as the review lens
...
- parent process owns self-review composition (implementer or main/goal agent)
- never a nested full `/self-review` skill subagent
...
- delegating the whole self-review skill to a worker that then must spawn lanes
...
- blind `hsm` lane sign-off (internal `self-review/hsm` pack) before the terminal readiness gate counts
