...
* before any Gabe-shaped agent calls `automation_update` or any available automation creation or update tool
  * [Load Gabe Automate](#load-gabe-automate)
...
* require watcher automations to refresh live state and execute the changed hot-path action instead of re-reading or restating Gabe context on every wake
...
* if `gabe-automate` is present in the active skill list
  ...
  * return to [Require Gabe Automate](#require-gabe-automate)
