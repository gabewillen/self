...
# copy carries GABE_WATCH_DETACHED=1 and runs the loop below. Detach mechanism,
# best first:
...
# GABE_WATCH_DETACH forces one mechanism (setsid|subshell|python3); default auto.
if [ -z "${GABE_WATCH_DETACHED:-}" ]; then
  case "$0" in
    /*) _gw_self="$0" ;;
    *)  _gw_self="$PWD/$0" ;;
  esac
  ...
  _gw_how="${GABE_WATCH_DETACH:-auto}"
  ...
  GABE_WATCH_DETACHED=1
  export GABE_WATCH_DETACHED
...
' "$_gw_bash" "$_gw_self" "$@" & ) </dev/null >/dev/null 2>&1
  ...
  echo "gabe-watch-ticker: could not detach (tried: $_gw_order)" >&2
  exit 1
fi
unset GABE_WATCH_DETACHED
# --- end self-daemonize -----------------------------------------------------
