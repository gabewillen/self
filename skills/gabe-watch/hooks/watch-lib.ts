...
/** List active gabe-watch MDScripts that have unprocessed ticks. */
...
export function formatActiveWatchSessionContext(): string {
  ...
  try {
    for (const project of readdirSync(projectsRoot)) {
      ...
      for (const file of readdirSync(goalsDir)) {
        if (!/^gabe-watch-.*\.mdscript\.md$/.test(file)) continue;
        const path = join(goalsDir, file);
  ...
  const parts = [
    "Active gabe-watch loops (Cursor): the detached ticker may still be writing ticks while the chat listener is dead.",
