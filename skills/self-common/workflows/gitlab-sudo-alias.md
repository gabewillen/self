...
* infer `{{self_role}}` from the caller when it is empty
...
* if the caller will prompt the user, a repository owner, or another authority surface for the missing alias decision
