...
* preserve the living-skills boundary: when a human correction, failed correction pattern, or named skill gap changes how future agents must write, edit, review, or coordinate, update the living skill pack in place (prefer `gabe-implement` and `gabe-review`, plus shared engineering-rules when both writers and reviewers must share the same MUST), strengthen or disambiguate the owning rule rather than leaving the correction only in chat, validate the edit, reinstall live, and publish the skill change when the live agents checkout allows it
* preserve the learn-stop boundary: every harness Stop hook must force a `/gabe-learn` pass via `gabe-common/workflows/gabe-learn.mdscript.md#reflect-and-learn` (MDScript only — not a skill role) before the turn may end; the agent reflects, updates living skills when a durable lesson exists, writes a satisfied learn stamp, and only then stops; opt out only with `GABE_LEARN_SKIP_HOOKS=1`
