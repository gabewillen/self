...
* preserve the learn-stop boundary: every harness Stop hook must force a `/gabe-learn` pass via `gabe-common/workflows/gabe-learn.mdscript.md#reflect-and-learn` (MDScript only — not a skill role) before the turn may end; the agent scans for **user-sourced** durable corrections only, updates project or global rules per scope, otherwise stamps `nothing-to-learn`, and only then stops; opt out only with `GABE_LEARN_SKIP_HOOKS=1`
