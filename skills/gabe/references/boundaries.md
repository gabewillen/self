...
* preserve the living-skills boundary: update the living skill pack only from **direct user** corrections that change how future agents must write, edit, review, or coordinate — never from the agent's own debugging, discoveries, tool failures, self-critique, or inferred best practices; prefer `gabe-implement` and `gabe-review` (plus shared engineering-rules when both need the same MUST), restate only what the user said, validate, reinstall live, and publish when the live agents checkout allows it
* preserve the learn-stop boundary: every harness Stop hook must force a `/gabe-learn` pass via `gabe-common/workflows/gabe-learn.mdscript.md#reflect-and-learn` (MDScript only — not a skill role) before the turn may end; the agent scans for **user-sourced** durable corrections only, updates living skills only when those exist, otherwise stamps `nothing-to-learn`, and only then stops; opt out only with `GABE_LEARN_SKIP_HOOKS=1`
