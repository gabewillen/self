...
* run [Select Configured Model And Reasoning](../self-common/workflows/model-reasoning-contract.md#select-configured-model-and-reasoning) with `{{self_role}}` set to `orchestrator`

* run [Resolve File Task Root](../self-common/workflows/file-task-comments.md#resolve-file-task-root)

* run [Read File Task Packet](../self-common/workflows/file-task-comments.md#read-file-task-packet) when a task file already exists for this lane
...
* run [Cleanup Created Threads](../self-common/workflows/thread-cleanup.md#cleanup-created-threads) before claiming any created chat thread, child thread, worker thread, or reviewer thread is terminal, superseded, or cleanly handed off
...
* before delegation, proof intake, stop reporting, or final decision
  ...
  * run [Add File Comment](../self-common/workflows/file-task-comments.md#add-file-comment) for the visible coordination record
  * run [Ensure File Plan](../self-common/workflows/file-task-comments.md#ensure-file-plan) when ordered work or a resumable plan is needed
  * run [Ensure File Instruction](../self-common/workflows/file-task-comments.md#ensure-file-instruction) when durable project instructions are added or changed

* when this is a project control-plane workflow and durable thread tooling is unavailable
  * run [Use Single Process Fallback](../self-common/workflows/file-task-comments.md#use-single-process-fallback)
...
* run [Maintain Lane Ledger](../self-common/workflows/lane-ledger.md#maintain-lane-ledger)

* run [Maintain File Lane Ledger](../self-common/workflows/file-task-comments.md#maintain-file-lane-ledger)
...
* run [Report Stop To File Comments](../self-common/workflows/file-task-comments.md#report-stop-to-file-comments)
...
* before claiming the root, child, or watched lane is terminal or superseded, run [Cleanup Created Threads](../self-common/workflows/thread-cleanup.md#cleanup-created-threads) for every chat thread this orchestrator created or inherited with cleanup ownership
...
* before asking the user, a repository owner, or another authority surface for input
  * run [Prepare Prompt Return Script](../self-common/workflows/return-script.md#prepare-prompt-return-script) with `{{return_source_workflow}}` set to this skill and `{{return_resume_heading}}` set to `stop-at-boundary`
