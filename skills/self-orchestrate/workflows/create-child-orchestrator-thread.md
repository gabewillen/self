...
* if the work is a single bounded implementation lane
  * stop and return to [Create Implementer Lane](create-implementer-lane.md#create-implementer-lane)
...
* run [Ensure File Task](../../self-common/workflows/file-task-comments.md#ensure-file-task) for the child orchestrator with `type: child-orchestrator`
* run [Write Goal MDScript](../../self-common/workflows/goal-mdscript.md#write-goal-mdscript) for the child orchestrator
* resolve `{{goal_mdscript}}` from the child goal path written above
* run [Add File Comment](../../self-common/workflows/file-task-comments.md#add-file-comment) on the parent task with the child handoff contract
...
* if thread creation tooling is unavailable
  ...
  * run [Use Single Process Fallback](../../self-common/workflows/file-task-comments.md#use-single-process-fallback) when the child lane can execute locally in the current run
...
* require the child to execute and report matching `{{event_exec}}` for `DISPOSITION_READY`, `TARGET_DRIFT`, `HANDOFF_UNACKED`, or `STALE_MR` via [Handle Thread Event Contracts](../../self-common/workflows/thread-event-contracts.md#handle-thread-event-contracts)
...
* update the ledger with [Maintain Lane Ledger](../../self-common/workflows/lane-ledger.md#maintain-lane-ledger)
