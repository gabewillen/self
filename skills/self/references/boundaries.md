...
* preserve the hard-cutover boundary: when code is pre-1.0 and not deployed to a production or user-facing environment, a replacement, rename, or migration must land as one cutover that deletes the replaced path, its tests, its configuration, and its documentation; deprecated shims, compatibility aliases, legacy fallbacks, version-suffixed duplicates, gating flags, and unreferenced leftovers are release-blocking unless the change names a released or deployed consumer that depends on the old path today and the condition that retires it

* preserve the atomic-commit boundary: every commit carries exactly one logical change, states that change in its subject, and builds and passes its governing checks on its own; unrelated formatting, refactors, dependency bumps, and drive-by fixes are committed separately, staging is deliberate rather than `git add -A` over a mixed tree, and checkpoint commits are squashed or amended before the branch is pushed
...
* preserve the forward-progress boundary: an agent executing MDScript writes MDScript as a running log — start it at the first context read, before the work, and update it at every transition that changes what is true, so the record survives a lost or compacted context instead of living in the conversation; each log carries `## Done So Far` as append-only history and `## Next Steps` as the executable states that remain, and its final state names the exact `/mdscript-exec <path>#<heading>` re-entry; name every one `<name>.mdscript.md`, sorting lexicographically in creation order (UTC stamp, zero-padded ordinal, subject, agent identity, kind), and never delete or overwrite an earlier one except to purge a leaked secret, which must also be rotated; the goal MDScript is this log for monitored, watch, goal, and automation lanes, and a single blind review lane is exempt because its one sign-off is its record
