...
* preserve the learn-stop boundary: every harness Stop hook must force a `/self-learn` pass via `self-common/workflows/self-learn.mdscript.md#reflect-and-learn` (MDScript only — not a skill role) before the turn may end; the agent scans for **user-sourced** durable corrections only, updates project or global rules per scope, otherwise stamps `nothing-to-learn`, and only then stops; opt out only with `SELF_LEARN_SKIP_HOOKS=1`
...
* preserve the review-fanout boundary: do not delegate the full `/self-review` skill to a subagent; the parent that can spawn owns composition and spawns only per-lane blind reviewers; the primary skill orchestrators hand to worker subagents is `/self-implement`
