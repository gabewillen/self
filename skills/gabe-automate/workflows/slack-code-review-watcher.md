...
* if `{{blocking_severity_threshold}}` is empty, set it to every unresolved finding that blocks the exact approval scope or is a Gabe-unacceptable maintainability smell
...
* if any Gabe-unacceptable maintainability smell exists below the configured severity threshold, continue at [Post Blocking Result](#post-blocking-result)

* record lower-severity findings below `{{blocking_severity_threshold}}` as nonblocking only when they do not block the exact approval scope and are not Gabe-unacceptable smells
...
* post one concise Gabe-voice blocker sentence in the original Slack thread
...
* post one concise Gabe-voice blocker sentence and the smallest useful question in the original Slack thread when delivery is authorized
