...
* delete stale `{{run_dir}}/review-verdict.json` and any `signoff-reviewer-*.json` before the fresh round
...
* persist `{{run_dir}}/review-verdict.json` only from the aggregated gabe-review decision (never invent Proven-for)
...
* if any lane failed or grade is `Not ready for …`
  ...
  * delete the three sign-offs and `review-verdict.json`
