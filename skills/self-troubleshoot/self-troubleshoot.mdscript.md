...
* run [Ensure File Task](../self-common/workflows/file-task-comments.mdscript.md#ensure-file-task) to resolve `{{task_id}}`, `{{artifact_dir}}`, and `{{return_dir}}` before any later state records evidence
...
* run [Confirm Safe Target](workflows/choose-environment.mdscript.md#confirm-safe-target) before touching a shared surface
...
* set `{{candidate_environment}}` to `{{target_environment}}`
* run [Confirm Safe Target](workflows/choose-environment.mdscript.md#confirm-safe-target) before removing anything from a shared target
...
* run [Add File Comment](../self-common/workflows/file-task-comments.mdscript.md#add-file-comment) with the blocker and the redacted evidence gathered so far
* if `{{parent_reporting_path}}` is set
