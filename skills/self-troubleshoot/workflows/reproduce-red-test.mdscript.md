...
* run [Confirm Safe Target](choose-environment.mdscript.md#confirm-safe-target) before any run against `{{target_environment}}`
...
* run [Log Progress](../../self-common/workflows/mdscript-artifact.mdscript.md#log-progress) with the confirmed reproduction and root-cause analysis as the next step
...
* set `{{mdscript_artifact}}` to `{{rca_mdscript}}`
* run [Log Progress](../../self-common/workflows/mdscript-artifact.mdscript.md#log-progress) with this failed reproduction attempt and the redacted mismatch
