...
* run [Ensure File Task](../../gabe-common/workflows/file-task-comments.md#ensure-file-task) for the child orchestrator with `type: child-orchestrator`
* run [Write Goal MDScript](../../gabe-common/workflows/goal-mdscript.md#write-goal-mdscript) for the child orchestrator
...
* if thread creation tooling is unavailable
  ...
  * run [Use Single Process Fallback](../../gabe-common/workflows/file-task-comments.md#use-single-process-fallback) when the child lane can execute locally in the current run
...
* require the child to execute and report matching `{{event_exec}}` for `DISPOSITION_READY`, `TARGET_DRIFT`, `HANDOFF_UNACKED`, or `STALE_MR` via [Handle Thread Event Contracts](../../gabe-common/workflows/thread-event-contracts.md#handle-thread-event-contracts)
