...
* if a root or coordinating Gabe thread owns the artifact
  * if the root or coordinating Gabe thread personally edits application code, owns ticket implementation, performs code review, spawns code reviewers, or treats coordinator inspection as the implementer's review gate
    * add a finding with consequence and evidence pointer
