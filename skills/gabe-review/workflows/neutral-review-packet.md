...
* run `/mdscript-exec {{review_skill_root}}/hsm/SKILL.md#triage` with `{{review_scope}}` from the in-scope paths when `{{review_skill_root}}` is set, otherwise `/mdscript-exec ~/.agents/skills/gabe-review/hsm/SKILL.md#triage`
