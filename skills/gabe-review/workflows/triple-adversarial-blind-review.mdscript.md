...
* if every lane in `{{blind_lanes}}` has `signed_off: true` and empty `p_findings`
  ...
  * set front matter to `reviewer_skill: "gabe-review"`, `multi_lane_blind: true`, `lanes` set to `{{blind_lanes}}`, `lane_selection_reasons`, `hsm_lane_verdict` when the HSM lane ran, `signoff_paths` for every lane file, `goal`, `conversation_id`, `run_id`, `proof_scope`, `grade`, `proof_decision`, `blocking_severities`, empty `blocking_findings`, `residual_findings`, `proof_supplied`, `proof_not_claimed`, `artifact_paths`, `commands_run`, `review_round`, and `reviewed_at`
