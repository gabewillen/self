...
* run [Use GitLab Sudo Alias Before Public Write](../../gabe-common/workflows/gitlab-sudo-alias.md#use-gitlab-sudo-alias-before-public-write) before writing any GitLab issue text, review response, or comment from this worker role
...
* do not keep an MR/PR in draft solely because CI/CD or checks are pending or failing; report check state separately and treat it as a default-branch merge blocker only when default-branch merge is the requested next action
...
* after creating or updating the MR/PR, if CI/CD, checks, review requests, reviewer grades, or unresolved discussions are pending
  * run [Create MR Monitor Goal](mr-monitor.md#create-mr-monitor-goal) before this implementer stops
