...
* set `{{rules_file}}` to `{{review_skill_root}}/references/engineering-rules/core.rules.md` when `{{review_skill_root}}` is set
* if `{{rules_file}}` is empty or missing
  * resolve `{{rules_file}}` from this file's directory as `../../../gabe-review/references/engineering-rules/core.rules.md`
