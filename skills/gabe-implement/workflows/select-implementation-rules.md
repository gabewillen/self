...
* set `{{review_skill_root}}` to `{{skills_root}}/gabe-review` when that directory exists
* otherwise set `{{review_skill_root}}` to `~/.agents/skills/gabe-review` when that directory exists
* if `{{review_skill_root}}` is still empty or missing
  * set `{{blocker}}` to `gabe-review skill root missing; cannot load engineering-rules packs`
  * stop and report that gabe-implement requires gabe-review's vendored engineering-rules
