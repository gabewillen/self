...
Rule files are **not** duplicated. Resolve each pack's `{{rules_file}}` under `gabe-review/references/engineering-rules/` (see [SOURCE.md](../../gabe-review/references/engineering-rules/SOURCE.md)).
...
Deep UML HSM semantic audit stays on the `gabe-review` `hsm` blind lane (`gabe-review/hsm/`). Implementers use `impl-hsm` as the construction checklist; they do not re-run the full adversarial HSM pack as a nested skill.
