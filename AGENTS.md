...
- Every harness Stop hook forces a `/self-learn` MDScript pass (`self-common/workflows/self-learn.mdscript.md#reflect-and-learn`) before the turn may end. Learn is not a skill role; it scans **user** corrections only and stamps `nothing-to-learn` when the user did not state a durable rule. Opt out only with `SELF_LEARN_SKIP_HOOKS=1`.
...
- Before completing code changes, the parent/orchestrator or implementer process owns `self-review` composition and fans out per-lane blind reviewers (never a nested full `/self-review` skill worker). Any parentless main agent is a root orchestrator.
