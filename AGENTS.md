...
- Executing MDScript means writing MDScript as a running log, not a summary at the end: start it at the first context read, update it at every transition that changes what is true, and keep `## Done So Far` (append-only) beside `## Next Steps` (executable) so a compacted or lost context resumes from disk. Every workflow that moves work forward emits or updates a durable `*.mdscript.md` under the project's artifact directory, named to sort lexicographically (UTC stamp, ordinal, subject, kind), append-only, whose states are the next executable steps and whose last state carries the exact `/mdscript-exec <path>#<heading>` re-entry. Author them with the `mdscript-write` conventions: one action per bullet, explicit branches, no prose.
